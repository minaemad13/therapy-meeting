...
class ParentsMeetingAdmin(ImportExportModelAdmin):
    ...
    # FIXED: 'send_ramadan_msg' is now fully lowercase to match the method name
    actions = ['send_meeting_link', 'send_ramadan_msg', 'send_alert']

    def send_meeting_link(self, request, queryset):
        self.message_user(request, "Send Messages running in the background. You can check the status from Message logs.")
        users_list = list(queryset)

        def run_bulk_send(users):
            batch_size = 10
            for i in range(0, len(users), batch_size):
                ...
                for obj in batch:
                    if obj.phone_number:
                        try:
                            sendMassage(obj.phone_number)
                        except Exception as e:
                            print(f"Error: {e}")
                time.sleep(2) # FIXED: Pauses between batches so you can safely send 200+ without rate limits

        threading.Thread(target=run_bulk_send, args=(users_list,)).start()
    ...
    def send_ramadan_msg(self, request, queryset):
        ...
        def run_bulk_send_ramadan(users):
            batch_size = 10
            for i in range(0, len(users), batch_size):
                batch = users[i:i + batch_size]
                for obj in batch:
                    if obj.phone_number:
                        try:
                            sendRamadanMsgGroup(obj.phone_number)
                        except Exception as e:
                            print(f"Error: {e}")
                time.sleep(2) 

        threading.Thread(target=run_bulk_send_ramadan, args=(users_list,)).start()
        ...
        def run_bulk_send_alert(users):
            ...
            for i in range(0, len(users), batch_size):
                ...
                for obj in batch:
                    if obj.phone_number:
                        try:
                            sendAlertMassage(obj.phone_number)
                        except Exception as e:
                            print(f"Error: {e}")
                time.sleep(2) 

        threading.Thread(target=run_bulk_send_alert, args=(users_list,)).start()
